...
*/
  ...
  generateToken () {
    return new Promise((resolve, reject) => {
      /*
      let jwt = new Jwt({
        crypto: { secret: this.Config.jwt.secret, algorithm: this.Config.jwt.algorithm },
        claims: {
          iat: true,
          nbf: Math.floor(Date.now() / 1000) - this.Config.validJitterSeconds,
          exp: Math.floor(Date.now() / 1000) + (this.Config.jwt.expireMinutes * this.Config.validJitterSeconds)
        }
      });
      */
